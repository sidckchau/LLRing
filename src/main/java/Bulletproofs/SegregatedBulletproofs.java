...
public class SegregatedBulletproofs {
    ...
    public SegregatedBulletproofs(int size, Pairing pairing, G1Point[] gs, G1Point[] hs, G1Point u) {
        G1 = pairing.getG1();
        this.gs=gs;
        this.hs=hs;
        this.u= u;
    ...
    public SegregatedBulletproofs(int size) {
    ...
    public Proof prove(BigInteger[] as, BigInteger[] bs ){
        ...
        pi.W1 = G1Point.mulAndSum(gs,as);
        pi.W2  = G1Point.mulAndSum(hs,bs);
    ...
    public void compress(G1Point[] g, G1Point[] h, BigInteger[] a, BigInteger[] b, Proof pi) {
        ...
        G1Point L1 =G1Point.mulAndSum(gR,aL);
        G1Point L2 = G1Point.mulAndSum(hL,bR).add(u.mul(cL));
        G1Point R1 = G1Point.mulAndSum(gL,aR);
        G1Point R2 = G1Point.mulAndSum(hR,bL).add(u.mul(cR));


        G1Point[] LR = {L1,L2,R1,R2};
...
//
        G1Point W1 = pi.W1;
        G1Point W2 = pi.W2.add(u.mul(pi.p));
        G1Point[] gprime = gs;
        G1Point[] hprime = hs;
...
//            BigInteger c = BigInteger.ONE;
            ...
            W1 = L1.mul(c2).add(W1).add(R1.mul(cinv2));
            W2 = L2.mul(c2).add(W2).add(R2.mul(cinv2));
        ...
        BigInteger theta = HashUtils.hash(gprime[0],hprime[0]).mod(n);

        G1Point result = gprime[0].mul(pi.a.multiply(theta).mod(n)).add(hprime[0].mul(pi.b)).add(u.mul(pi.a.multiply(pi.b)));



        return result.isEqual(W1.mul(theta).add(W2));
...
//    }
    ...
    public static void main(String[] args) {
        ...
        int size =16;
        SegregatedBulletproofs bp = new SegregatedBulletproofs(size);
...
//        BigInteger [] t2 = swift.hadamardProd(t,t1,swift.n,BigInteger.ONE.negate());
        ...
        System.out.println(bp.verify(pi));
    }
}
